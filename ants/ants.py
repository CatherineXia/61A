...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        "*** YOUR CODE HERE ***"
        if ( not self.place.ant) or (self.place.ant is not None and self.place.ant.blocks_path == False):
            return False
...
class Ant(Insect):
    ...
    def can_contain(self, other):
        if self.container == True and self.ant is None and other.container == False:  #ant.container possibility 
            return True
        else:
            return False
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        location = self.place
        index = 0
        while location.entrance is not None or location != hive:
            if len(location.bees) > 0 and self.min_range <= index <= self.max_range:
...
class QueenAnt(ScubaThrower):
    ...
    def action(self, colony):
        ...
        # Impotently double the damage of fellow ants in the queen's tunnel.
