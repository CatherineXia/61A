...
def make_tweet_fn(text, time, lat, lon):
    ...
    def tweet(str):
        if str == 'text':
            ...
            print("Your information can not found.")
    return tweet
...
def find_centroid(polygon):
    ...
    """
    "*** YOUR CODE HERE ***"
    sum_of_area, center_x, center_y, i = 0, 0, 0, 0
    while i < len(polygon) - 1:
        x1, y1 = latitude(polygon[i]), longitude(polygon[i])
        x2, y2 = latitude(polygon[i + 1]), longitude(polygon[i + 1])
        sum_of_area += x1 * y2 - x2 * y1
        center_x += (x1 + x2) * (x1 * y2 - x2 * y1)
        center_y += (y1 + y2) * (x1 * y2 - x2 * y1)
        i += 1
    sum_of_area /= 2
    if sum_of_area == 0:
        return latitude(polygon[0]), longitude(polygon[0]), sum_of_area
    center_x = center_x / (6 * sum_of_area)
    center_y = center_y / (6 * sum_of_area)
    return center_x, center_y, abs(sum_of_area)
...
def find_state_center(polygons):
    ...
    """
    "*** YOUR CODE HERE ***"
    sum_of_area, average_x, average_y = 0, 0, 0
    for i in polygons:
        average_x += find_centroid(i)[0] * find_centroid(i)[2]
        average_y += find_centroid(i)[1] * find_centroid(i)[2]
        sum_of_area += find_centroid(i)[2]
    average_x = average_x / sum_of_area
    average_y = average_y / sum_of_area
...
def average_sentiments(tweets_by_state):
    ...
    for state in tweets_by_state.keys():
        if individual_average(tweets_by_state[state]) != 0:
            averaged_state_sentiments[state] = individual_average(tweets_by_state[state])
